...
- 不修复与 V2 无关的全量测试故障（taskboard 已随剥离移除）。
- 不使用通用错误文案猜测 5 小时额度。
...
- `npm run build`：通过。
- `npm --workspace apps/taskboard run typecheck`：通过（taskboard workspace 已随剥离移除；宿主侧等价自检为 `npm run check:host`）。
- `npm test`：存在 2 个 Windows `EBUSY` 基线失败，来源是 app-server 子进程 kill 后未等待退出。
- taskboard `node --test`：存在 3 个基线失败，其中 2 个为 `EBUSY`，1 个为测试读取本机 23 个项目而非隔离状态（该用例随 taskboard 剥离已移除，宿主侧等价覆盖见 apps/resume-host/tests/host.test.mjs）。
...
| T4 | `apps/resume-host/src/daemon.mjs`、`apps/resume-host/src/http-server.mjs` 接线、宿主侧 V2 测试（原 `apps/taskboard/server/supervisor-api.mjs`、`app.mjs` 的 V2 路由分发已随 taskboard 移除） |
| T5 | `apps/resume-host/ui/app.js`（只读状态页；watch 增删由 CLI `resume-host watch add/rm` 承担）。原 `apps/taskboard/web/src/api.ts`、`SupervisorView.tsx` 已随 taskboard 移除 |
...
禁止：读取真实 `%LOCALAPPDATA%`、spawn Codex、修改 resume-core 之外的代码。

验证：

```powershell
npm run typecheck
npx vitest run tests/auto-resume-reducer.test.ts tests/five-hour-quota.test.ts
npm run lint
```
...
```powershell
npm run typecheck
npx vitest run tests/auto-resume-store.test.ts tests/auto-resume-monitor.test.ts tests/resume-attempt.test.ts
```
...
```powershell
npm run typecheck
npx vitest run tests/desktop-session-observation.test.ts
```
...
- 宿主启动立即 detect，之后 180 秒 fixed-rate；进程内不可重入。
- `AUTO_RESUME_V2_EXECUTE` 默认关闭；未识别 300 分钟窗口、检测到 legacy daemon owner 或配置冲突时拒绝执行发送。
- V2 execute 开启时，宿主侧不存在 60 秒 auto-resume job tick（原 taskboard 的 60 秒执行循环已随剥离移除）；旧 jobs 不被删除也不被改写，只在 execute 能力门禁里被只读检查（存在活跃 legacy job 即阻断发送）。
...
```powershell
npm run build
node --test apps/resume-host/tests/host.test.mjs
npm run check:host
```

宿主侧说明：原 `apps/taskboard/test/auto-resume-v2-server.test.mjs`（该用例随 taskboard 剥离已移除，宿主侧等价覆盖见 `apps/resume-host/tests/host.test.mjs`）。

### T5：前端勾选与状态展示

剥离后注记：本节描述的 taskboard 前端已随剥离移除；宿主侧现为只读状态页 `apps/resume-host/ui/app.js`（5 秒轮询 `/api/host/status` 与 `/api/auto-resume/watches`，没有增删 watch 的入口，仅「立即检测」按钮；watch 增删由 CLI `resume-host watch add/rm` 或 `/api/auto-resume/*` 承担）。下列条款作为当时的实施合同保留。
...
宿主侧说明：原 `apps/taskboard/web/src/components/SupervisorView.test.tsx`（该用例随 taskboard 剥离已移除，宿主侧等价覆盖见 `apps/resume-host/tests/host.test.mjs`）。
...
宿主侧说明：原 `npm --workspace apps/taskboard run typecheck`、`npm --workspace apps/taskboard run test`、`npm run build:taskboard` 随 taskboard 剥离已移除（该 workspace 不再存在）。

若 build 会刷新正在运行的宿主所依赖的 resume-core 产物，员工不得自行执行，需报告并由领导决定安全窗口。
...
- 其次：停止唯一 scheduler owner；需要只读观察时改用 `apps/resume-host/start-observe.cmd`（observe 模式，零发送）。
...
- `CODE_PASS`：通过。最终独立复审确认 0 个 blocker/major；服务端 V2 34/34、monitor 30/30、Desktop observation 28/28、相关 core 65/65，resume-core/taskboard typecheck、resume-core build 与 taskboard web build 均通过（taskboard 相关项随剥离移除，计数为剥离前历史记录）。`adopt.test.ts` 单跑 1/1、`cli-e2e.test.ts` 单跑 3/3；core 全量最新复跑 147/148，唯一失败是 `cli-e2e` 完成产品断言后的 Windows 临时目录清理 `EBUSY`。
- `UI_PASS`：通过。隔离状态目录、`AUTO_RESUME_V2_EXECUTE=0`、`AUTO_RESUME_V2_SHADOW=1` 下，用真实应用内浏览器完成：加载 104 个真实会话 → 勾选已完成会话 → 等待真实固定 180 秒 tick → 服务端持久化 `sessionState=COMPLETED`、`fiveHourQuota=UNKNOWN`、`lastError=null` 与实际检测时间 → 页面显示“监控中/UNKNOWN/上次检测” → 人工取消 → API 确认 watch 数量归零。整个过程未产生 resume attempt。（本次 UI 验收在剥离前的 taskboard 前端上完成；该前端已随 taskboard 移除，宿主侧现为只读状态页 `apps/resume-host/ui/app.js`，watch 增删走 CLI/HTTP）
...
- 范围外已知噪声：Taskboard AI 模型目录仍可能调用失效的全局 npm `codex.js debug models`（该组件已随 taskboard 剥离移除）；取证确认它不属于 V2 quota/session/sender 调用链，不影响上述自动续跑验收，留给独立任务处理。
