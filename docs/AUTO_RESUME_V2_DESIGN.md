...
- `src/constants.ts` 当前默认调度周期是 60 秒，不是 3 分钟。
- `src/supervisor.ts:12-76` 会处理 `running` 状态，并可能重新发起执行。
- `src/supervisor.ts:31` 只有 `waiting_rate_limit` 才被识别为 resume 路径。
- `src/types.ts:11-27` 没有“上一轮进行中 + 额度可用”的持久化快照。
- `src/app-server/supervisor.ts:41` 只在实际执行前读取一次额度；CLI 路径不主动读取额度。
- `src/desktop-sessions.ts:131-137` 无法把真实的通用 usage-limit 文案可靠区分为 5 小时额度或周额度。
- `src/lock.ts` 只按 job 加锁；同一 `threadId` 的两个 job 可以分别获得锁并重复续跑。
- 原 taskboard 前端组件 `apps/taskboard/web/src/components/SupervisorView.tsx`（该组件已随 taskboard 剥离移除；新宿主的展示面是只读状态页 `apps/resume-host/ui/app.js`）的 15 秒刷新只能负责展示，不能成为状态机的采样来源。
...
- 宿主（resume-host）启动后立即检测一次，此后按 fixed-rate 每 180 秒产生一个检测时点。
- 同一进程最多一轮在运行；到达检测时点而上一轮未结束时，本次记为 `SKIPPED_OVERLAP`，不叠加补跑，下一次仍在后续 180 秒边界触发。
- 宿主内嵌调度器是唯一默认 owner；独立 daemon 启动时必须通过全局 owner lease 竞争 owner，不能与宿主同时调度。
- V2 execute 开启时，旧 CLI daemon 和 taskboard 时代 60 秒的 job 执行循环必须退出自动续跑职责（taskboard 已随剥离移除，宿主侧不再有 60 秒 job 执行循环，遗留 job 只由 execute 能力门禁只读阻断）；CLI 若要提供 V2 守护入口，也必须竞争同一全局 owner lease。
...
| 文件/模块 | 变更职责 | 禁止事项 |
|---|---|---|
| `src/auto-resume-types.ts`（新） | watch、snapshot、attempt 类型 | 不读取 UI 状态 |
| `src/auto-resume-reducer.ts`（新） | 纯函数状态机 | 不发 RPC、不写盘 |
| `src/five-hour-quota.ts`（新） | 300 分钟窗口识别与规范化 | 不用通用文案猜 5h |
| `src/auto-resume-store.ts`（新） | watch 原子持久化、schema 迁移 | 不覆盖旧 jobs |
| `src/auto-resume-monitor.ts`（新） | 3 分钟检测、批量决策、队列 | running 时禁止调用 resume |
| `src/resume-attempt.ts`（新） | thread 锁、幂等发送、确认 | 不按 jobId 去重 |
| `src/desktop-sessions.ts` | 输出可信规范状态 | 不承担业务迁移 |
| `apps/resume-host/src/daemon.mjs`（模块名 `createResumeDaemon`；HTTP 面见 `apps/resume-host/src/http-server.mjs`；原 `apps/taskboard/server/supervisor-api.mjs` 已随 taskboard 移除，其 V1 job 能力在宿主里不存在） | V2 watch API、唯一 scheduler owner | 不保留 60 秒自动续跑 tick |
| `apps/resume-host/ui/app.js`（原 `apps/taskboard/web/src/components/SupervisorView.tsx` 已随 taskboard 移除；watch 增删由 `resume-host watch add/rm` 与 `/api/auto-resume/*` 承担） | phase 展示 | 刷新不得触发状态迁移 |

不要在同一实现 wave 中让多个员工同时修改 `apps/resume-host/src/daemon.mjs` 或 `apps/resume-host/ui/app.js`。先落纯状态机和存储契约，再接服务端，最后接 UI。
