...
你让 Codex 跑长任务，额度用完后它停在半路；额度重置时，你可能不在电脑前，任务便一直搁着。这个仓库让你选定需要守护的会话，确认它确实因额度中断后，等额度恢复再**继续同一个 Codex 会话**，不用守着重置时间手动发“继续”。

**Codex 无人值守续跑 + 项目总谱 + API 配额** —— 续跑之外，还能查看任务是否真的在运行、多个项目各进行到哪里、常用 AI 服务还剩多少余额或用量。正常完成或额度信息不确定时不会擅自续跑；后台执行需显式启用。
